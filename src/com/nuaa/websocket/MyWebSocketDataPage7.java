...
@ServerEndpoint("/chat7.ws")
public class MyWebSocketDataPage7 {
	// 静态变量，用来记录当前在线连接数。应该把它设计成线程安全的。
	private static int onlineCount = 0;

	// concurrent包的线程安全Set，用来存放每个客户端对应的MyWebSocket对象。若要实现服务端与单一客户端通信的话，可以使用Map来存放，其中Key可以为用户标识
	private static CopyOnWriteArraySet<MyWebSocketDataPage7> webSocketSet = new CopyOnWriteArraySet<MyWebSocketDataPage7>();
	...
	public void onOpen(Session session) {
		MyWebSocketDataPage7.session = session;
	...
	public static synchronized void addOnlineCount() {
		MyWebSocketDataPage7.onlineCount++;
	}
	...
	class Runnable1 implements Runnable{
	    public void run() {
	        for (int i = 0; i < 100; i++) {
	        	if(MyWebSocketDataPage7.onlineCount>0){
		            String data="";
					try {
