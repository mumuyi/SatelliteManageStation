...
@ServerEndpoint("/chat6.ws")
public class MyWebSocketDataPage6 {
	// 静态变量，用来记录当前在线连接数。应该把它设计成线程安全的。
	private static int onlineCount = 0;

	// concurrent包的线程安全Set，用来存放每个客户端对应的MyWebSocket对象。若要实现服务端与单一客户端通信的话，可以使用Map来存放，其中Key可以为用户标识
	private static CopyOnWriteArraySet<MyWebSocketDataPage6> webSocketSet = new CopyOnWriteArraySet<MyWebSocketDataPage6>();
	...
	public void onOpen(Session session) {
		MyWebSocketDataPage6.session = session;
		...
		if(flag==0){
			...
			list = MyHibernate.sqlQueryWithCondition("from Parameter where Sort=?", "6");
			flag=1;
		}
	...
	public static synchronized void addOnlineCount() {
		MyWebSocketDataPage6.onlineCount++;
	}
