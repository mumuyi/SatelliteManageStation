...
public class PrepareData {
	 ...
	 * 按格式获取历史记录查询的数据;
	 * 
	 * DataTable 查询的表; page 查询数据的页数; rows 一页所需显示数据条数; Sort 数据类别; Sort1 数据类别1;
	 * 
	 ...
	 * 按格式获取历史记录查询的数据且实现排序;
	 * 
	 * sidx 所需排序的列的名称; sord 排序方式,desc或者acs;
	 */
	 ...
	 * 按格式获取实时显示数据的表格形式数据;
	 * 
	 * type==0 时,带最大值最小值; type==1时,不带最大最小值;
	 */
	public String getTableShowData(String DataTable, int page, int rows, int Sort, int Sort1, int type)
			throws NoSuchMethodException, SecurityException, IllegalAccessException, IllegalArgumentException,
			InvocationTargetException {
		// 传过来的参数 page 是从1开始的,所以要减一;
		page -= 1;
	 ...
	 * 
	 * */
	public String prepareDynamicDisplayData(int temp,List <?> framelist,List<?> list) throws NoSuchMethodException, SecurityException, IllegalAccessException, IllegalArgumentException, InvocationTargetException{
		DataKind dk=new DataKind();
		MyNerveNet nn=new MyNerveNet();
		String ans="[{";
		// 填充数据;
		...
		for (int j = 0; j < list.size(); j++) {
			...
			if(dk.getCurrentDataKind()==1||dk.judgeDataPredictValue(parameter.getName())==1){
				//显示实时数据;
				Method m = (Method) myclass.getMethod("get" + DataOpration.captureName(parameter.getName()));
				ans +="" + (m.invoke(frame));
			}else{
				//显示预测数据;
				ans+=""+nn.getPredictData(parameter.getName(), temp+200);
			}
